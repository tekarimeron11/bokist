...
| R-043 | **問題の結果画面**に、当該問題の関連記事リンクを最大3件表示する（学習動線の中核）。優先順位は **`explanationStructured.relatedSlugs`（明示指定・最優先）→ `topicId` 一致 → `chapterId` 一致 → 0件で「準備中」表示**（§7.7 順方向アルゴリズム参照）。未存在 slug は silent drop。 | 必須 |
...
#### `relatedSlugs` 検証規則
- **件数**: 存在する場合は **1〜3件**。空配列は不可（要素必要なら省略）。4件以上はビルドエラー。
- **形式**: 各 slug は ASCII kebab-case（`[a-z0-9-]+`）。正規表現違反はビルドエラー。
- **重複**: 同一 slug 重複不可。重複検出はビルドエラー。
- **未知 slug**: ランタイムでは `findArticleBySlug` 解決失敗時に **silent drop**（QuizScreen の関連記事一覧から除外、警告も出さない）。ビルドは通す（writer の typo / 先行参照を許容）。
- **空 / 未指定**: `undefined` または省略は許容。§7.7 順方向アルゴリズムが topicId / chapterId フォールバックで補完。
...
3. すべての `Question.explanationStructured.<field>` に含まれる `<Term>X</Term>` の `X` が、`GlossaryTerm.term` の集合に含まれること（孤立Termを禁止）。違反時はビルドエラー。
...
| checklist | `night-before-exam` | 試験前日チェックリスト |
...
`category: 'basics'` の9本（既存 `5-categories` + M4.1 新規8本）が**基礎知識体系**を構成し、解説テンプレと StructuredExplanation の `relatedSlugs` から最も頻繁に参照される。
...
結果画面に表示する解説は、初学者が「読んだのに意味がわからない」状態に陥らないことを最優先する。本仕様は §7.1 `Question.explanationStructured`（StructuredExplanation）の表示・操作・検証を定義する。
...
- 各 `Question.explanationStructured` は M2 リリース時に全52問で必須化。M2 の `<Term>` タグは「将来用にマークアップは入れておくが押せない」状態を許容する。
